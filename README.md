...
A 53-node algebraic intermediate language with explicitly-typed arithmetic, indexed Σ/Π loops, FFT/IFFT, tuples, Fiat-Shamir prover/verifier, and explicit type coercions over BLS12-381 field/curve/polynomial types, optimized via egg's equality saturation with type-analysis-guarded rewrite rules.
...
| **Type Tags** (14) | `Field`, `Curve`, `Int`, `Bool`, `DensePoly`, `SparsePoly`, `DenseMLE`, `SparseMLE`, `MVPoly`, `Array`, `Pair`, `ProverState`, `VerifierState`, `arrayof` | `Field`, `(arrayof Field)` |
...
| **Sponge — Prover** (3) | `absorb_public`, `absorb_private`, `squeeze` | `(absorb_public Field s x)`, `(squeeze Field s)` |
| **Sponge — Verifier** (3) | `read_transcript`, `verify`, `check_eof` | `(read_transcript Field vs)`, `(verify cond)` |
...
### Fiat-Shamir Sponge (Prover & Verifier)

Cryptographic sponge for Fiat-Shamir transformations, backed by [spongefish](https://github.com/arkworks-rs/spongefish). Sponge values are **linear resources** — each can be used exactly once (enforced at runtime). Prover and verifier are **separate programs** with distinct sponge types.

| Operation | Prover (`ProverState`) | Verifier (`VerifierState`) |
|-----------|----------------------|--------------------------|
| Public absorb | `(absorb_public T ps val)` → ProverState | `(absorb_public T vs val)` → VerifierState |
| Private absorb | `(absorb_private T ps val)` → ProverState | ❌ not available |
| Read transcript | ❌ not available | `(read_transcript T vs)` → Pair(T, VerifierState) |
| Squeeze challenge | `(squeeze T ps)` → Pair(T, ProverState) | `(squeeze T vs)` → Pair(T, VerifierState) |
| Assert | — | `(verify bool_expr)` → Bool or error |
| EOF check | — | `(check_eof vs)` → Bool or error |

```lisp
;; ── Prover ──
(let s1 (absorb_public Field s0 public_input)
  (let s2 (absorb_private Field s1 witness)
    (squeeze Field s2)))                       ;; → Pair(challenge, ProverState)

;; ── Verifier ──
(let vs1 (absorb_public Field vs0 public_input)
  (let r (read_transcript Field vs1)           ;; read witness from transcript
    (let witness (fst r)
      (let vs2 (snd r)
        (let r2 (squeeze Field vs2)
          (let challenge (fst r2)
            (let vs3 (snd r2)
              (let eof (check_eof vs3)
                (verify (eq Field expected actual))))))))))
```
...
```bash
cargo build --release
cargo test --release       # 331 tests (250 lib + 44 algebraic laws + 37 property)
...
- **250 unit tests**: evaluator, language parsing, rewrite rules, type analysis, guarded conditions, FFT/IFFT, symbolic poly, tuples, coerce, typed ops, for comprehension, stream fusion, MLE partial eval, sponge Fiat-Shamir, prover/verifier roundtrip
